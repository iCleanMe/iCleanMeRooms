...
final class RoomDetailViewModelTests: XCTestCase {
    ...
    func test_guest_cannot_add_room_when_current_room_count_exceeds_guest_limit() {
        let roomCount = 5
        let (sut, _) = makeSUT(room: .init(), user: makeRoomUser(type: .guest), roomCount: roomCount)
    ...
    func test_normal_user_cannot_add_room_when_current_room_count_exceeds_normal_limit() {
        let roomCount = 8
        let (sut, _) = makeSUT(room: .init(), roomCount: roomCount)
    ...
    func test_pro_user_can_always_add_a_room() {
        (0...20).forEach { roomCount in
            let (sut, _) = makeSUT(room: .init(), user: makeRoomUser(type: .pro), roomCount: roomCount)
...
extension RoomDetailViewModelTests {
    func makeSUT(room: Room = .init(), user: RoomUser? = nil, roomCount: Int = 1, throwError: Bool = false, updateRoom: @escaping (Room) async throws -> Void = { _ in }, saveNewRoom: @escaping (Room) async throws -> Void = { _ in }, file: StaticString = #filePath, line: UInt = #line) -> (sut: RoomDetailViewModel, delegate: MockDelegate) {
        let delegate = MockDelegate(throwError: throwError)
        let datasource = RoomDataSource(user: user ?? makeRoomUser(), sections: [.init(id: "", name: "", rooms: createRooms(count: roomCount))])
